...
void Utils::getSecureRandom(void* buf, unsigned int bytes)
{
	...
	static uint8_t randomBuf[256];
	static unsigned int randomPtr = 256;
	...
	for (unsigned int i = 0; i < bytes; ++i) {
		if (randomPtr >= 256) {
			...
			if (! CryptGenRandom(cryptProvider, (DWORD)256, (BYTE*)randomBuf)) {
				fprintf(stderr, "FATAL ERROR: Utils::getSecureRandom() CryptGenRandom failed!\r\n");
				exit(1);
			}
			randomPtr = 0;
			s20.crypt12(randomBuf, randomBuf, 256);
	...
	for (unsigned int i = 0; i < bytes; ++i) {
		if (randomPtr >= 256) {
			for (;;) {
				if ((int)::read(devURandomFd, randomBuf, 256) != (int)256) {
					::close(devURandomFd);
					devURandomFd = ::open("/dev/urandom", O_RDONLY);
					if (devURandomFd < 0) {
						fprintf(stderr, "FATAL ERROR: Utils::getSecureRandom() unable to open /dev/urandom\n");
